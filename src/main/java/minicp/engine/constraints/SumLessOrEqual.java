...
/**
 * SumLessOrEqual Constraint
 */
public class SumLessOrEqual extends AbstractConstraint {
     ...
     */
    public SumLessOrEqual(IntVar[] x, IntVar y) {
     ...
     * @param y the right hand side of the sum
     */
    public SumLessOrEqual(IntVar[] x, int y) {
        this(Arrays.copyOf(x, x.length + 1));
     ...
     */
    public SumLessOrEqual(IntVar[] x) {
        super(x[0].getSolver());
        this.x = x;
        this.n = x.length;
        min = new int[x.length];
        nFixed = getSolver().getStateManager().makeStateInt(0);                     // number of fixed variables
        sumFixed = getSolver().getStateManager().makeStateRef(Long.valueOf(0));   // sum of fixed variables
